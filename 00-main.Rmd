...
- Exposure to many Epidemiology concepts, collaboration with physicians
  - Case-cohort design, Mediation analysis, Competing risk models
:::
...
# MOTE RF
### Motivation
A dietary study on the health benefit of consume whole almond versus processed almond
...
How to model individualized treatment effect given microbiome data?
$$
...
- Treatment effect function $\bs f(\cdot, \cdot): \mathcal{X} \times \mathcal{A} \rightarrow \mathcal{Y}$
...
- Modified outcomes $\Delta \bs Y A$ as response of the proposed model
  ...
  $$
- Oblique split rule
  - A linear combination of predictors, $\bs \alpha ^T \bs X^e$, as _splitting variable_
  - Variance reduction on a pusdo-response, i.e. dimension-reduced modified outcomes, $\bs\kappa^T\Delta Y$
  - Two unknown vectors, $\bs\alpha$ and $\bs\kappa$, are calculated locally using  multi-block Canonical Correlation Analysis (CCA)
...
- Prognostic modeling
  - Integrating clinical variables with -omics data


### Random Forests for -Omics Data Analysis
- Assessment of classical random forests in high-dimensional settings
